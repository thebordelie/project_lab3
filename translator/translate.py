...
stack: dict[str, int] = {}
stack_pointer = 0
memory_pointer = MAX_INSTRUCTIONS_SIZE + 1
...
def add_ld_relative_instr(address: int) -> None:
    add_instructions(Opcode.LD, TypesOfAddressing.RELATIVE.value, address)


def add_st_relative_instr(address: int) -> None:
    add_instructions(Opcode.ST, TypesOfAddressing.RELATIVE.value, address)
...
def add_output_instr() -> None:
    add_instructions(Opcode.PRINT, None, Port.OUT.value)
...
def process_input_node(node: InputOutputNode) -> None:
    ...
    add_instructions(Opcode.READ, None, Port.BUFFER_IN.value)
...
def translate_node(ast_tree: Node) -> None:
    node_handler[ast_tree.type](ast_tree)
...
def reset_param():
    global stack_pointer, memory_pointer
    commands.clear()
    stack.clear()
    stack_pointer = 0
    memory_pointer = MAX_INSTRUCTIONS_SIZE + 1
