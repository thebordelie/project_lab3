...
def parse_var_init(tokens: deque[Token]) -> Node:
    ...
    if token.token_type.name == TypeName.ID:
        ...
        token = tokens.popleft()
        if token.token_type.name == TypeName.LEFT_SQUARE_BRACKET:
            buffer = tokens.popleft().text
            tokens.popleft()
        else:
            buffer = 1
        if token.token_type.name == TypeName.EQUAL:
...
def parse_if_statement(tokens: deque[Token]) -> Node:
    ...
    while tokens[0].token_type.name != TypeName.RIGHT_BLOCK:
        if tokens[0].token_type.name == TypeName.LEFT_BLOCK:
            tokens.popleft()
            continue
...
def parse_input_output_statement(tokens: deque[Token]) -> InputOutputNode:
    ...
    input_output_type = InputOutputType.OUTPUT if action == "print" else InputOutputType.INPUT
    tokens.popleft()
